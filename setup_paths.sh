...
echo "=========================================="
echo "Setting up paths for new machine"
echo "更新路徑以適配新電腦"
echo "=========================================="
...
if [ ! -d "$DATASET_DIR" ]; then
    ...
    echo "Please download the dataset first. / 請先下載資料集"
    exit 1
fi
...
echo "Updating .env file PROJECT_ROOT..."
# Use Python for cross-platform path handling / 使用 Python 進行跨平台路徑處理
python3 << EOF
...
env_path = str(project_root).replace("\\", "/")
...
# Old prefixes to replace / 需要替換的舊前綴
old_prefixes = [
    "/root/ultrasound/DifficultAirway/",
    "D:/workplace/project_management/github_project/ultrasound-airway-detection/",
    "D:/workplace/project_management/github_project/ultrasound-airway-detection2/",
...
if changed:
    ...
    with open(split_file, 'w', encoding='utf-8') as f:
        for line in new_lines:
            f.write(line + "\n")
    print(f"  ✅ Updated: {split_file.name}")
else:
    print(f"  ℹ️  No changes needed: {split_file.name}")
EOF
        done
        
        # Process YAML files / 處理 YAML 檔案
        for yaml_file in "$dataset_version_dir"/*.yaml; do
            if [ ! -f "$yaml_file" ]; then
                continue
            fi
            
            yaml_name=$(basename "$yaml_file")
            echo "  Processing YAML: $yaml_name"
