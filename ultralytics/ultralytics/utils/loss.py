...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        if self.dfl_loss:
            ...
            if self.use_dim_weights:
                # Compute DFL loss for each dimension [l, t, r, b] separately with weights
                # pred_dist shape: (batch, anchors, reg_max * 4)
                # target_ltrb shape: (batch, anchors, 4)
                pred_dist_reshaped = pred_dist[fg_mask].view(-1, 4, self.dfl_loss.reg_max)  # (n_fg, 4, reg_max)
                target_ltrb_fg = target_ltrb[fg_mask]  # (n_fg, 4)
                
                # Compute DFL loss for each dimension separately
                loss_dfl_per_dim = []
                for dim_idx in range(4):  # [l, t, r, b]
                    dim_loss = self.dfl_loss(
                        pred_dist_reshaped[:, dim_idx, :],  # (n_fg, reg_max)
                        target_ltrb_fg[:, dim_idx:dim_idx+1]  # (n_fg, 1)
                    )  # Returns (n_fg, 1)
                    
                    # Apply dimension-specific weight
                    dim_loss = dim_loss * self.dim_weights[dim_idx]
                    
                    loss_dfl_per_dim.append(dim_loss)
                
                # Combine losses from all dimensions: (n_fg, 4)
                loss_dfl = torch.cat(loss_dfl_per_dim, dim=1)
                loss_dfl = (loss_dfl * weight).sum() / target_scores_sum
            else:
                # Original implementation: compute DFL loss for all dimensions together
...
class RotatedBboxLoss(BboxLoss):
    ...
    def __init__(
        ...
        super().__init__(reg_max, use_dim_weights=use_dim_weights, dim_weights=dim_weights)
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        Args:
            model: Model instance (must be de-paralleled).
            tal_topk (int): Top-k candidates for task-aligned assignment.
            use_dim_weights (bool, optional): Whether to use dimension-specific weights for bbox loss.
                                             If None, reads from model.args.use_dim_weights (default: False).
            dim_weights (List[float], optional): Weights for [left, top, right, bottom] dimensions.
                                                 If None, reads from model.args.dim_weights.
                                                 Only used if use_dim_weights=True.
                                                 Example for det_123 (w and x important): [2.0, 1.0, 2.0, 1.0]
                                                 Example for det_456 (h and y important): [1.0, 2.0, 1.0, 2.0]
            use_focal_loss (bool, optional): Whether to use Focal Loss instead of BCE Loss.
    ...
    def __call__(self, preds: Any, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_focal_loss and self.focal_loss is not None:
            # Use Focal Loss for classification (better for small objects and class imbalance)
            loss[1] = self.focal_loss(pred_scores, target_scores.to(dtype)) / target_scores_sum
        else:
            # Use BCE Loss (default)
